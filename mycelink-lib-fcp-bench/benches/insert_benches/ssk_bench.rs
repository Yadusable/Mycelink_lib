...
pub fn ssk_bench(c: &mut Criterion) {
    ...
    .bench_function("ssk_named_bench", |b| {
        b.to_async(
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
...
async fn ssk_bench_named_fn() {
    ...
    let insert_uri = format!("{}name", keypair.insert_uri);
    let request_uri = format!("{}name", keypair.request_uri);
