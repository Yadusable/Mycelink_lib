...
pub async fn measure_usk_update_time_10(mut put_message: ClientPutMessage, receive_uri: URI) {
    let mut conn_1 = prepare_connection(FCP_PORT_1).await;
    let mut conn_2 = prepare_connection(FCP_PORT_2).await;
    ...
    for i in 0..10 {
        put_message.identifier = UniqueIdentifier::new(format!("Update {i}/10"));
        let mut data = [0; 1024];
        rand::thread_rng().fill_bytes(&mut data);
        put_message.upload_from = Direct { data: data.into() };
            ...
            match message.message_type() {
                ...
                MessageType::Node(message_type) => match message_type {
                    NodeMessageType::AllData => {
                        ...
                        if (Direct { data: msg.data }) == put_message.upload_from {
                            break;
                        }
                    }
                    NodeMessageType::DataFound => {}
                    ...
                    NodeMessageType::SubscribedUSKUpdate => {
                        ...
                        let get = ClientGetMessage {
                            identifier: UniqueIdentifier::new("get update usk {i}/10"),
                            uri: update.uri,
                            verbosity: Default::default(),
                            return_type: ReturnType::Direct,
                            max_size: None,
